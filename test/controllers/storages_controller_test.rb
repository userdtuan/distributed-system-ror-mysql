...
class StoragesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create storage" do
    assert_difference("Storage.count") do
      post storages_url, params: { storage: { address: @storage.address, available_capacity: @storage.available_capacity, capacity: @storage.capacity, name: @storage.name } }
    end
  ...
  test "should update storage" do
    patch storage_url(@storage), params: { storage: { address: @storage.address, available_capacity: @storage.available_capacity, capacity: @storage.capacity, name: @storage.name } }
